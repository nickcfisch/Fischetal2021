...
!!ad_comm::change_datafile_name(datafile);
  ...
  init_vector obs_fishery_cpue(fyear,lyear)	//observed fishery effort
...
PARAMETER_SECTION
  ...
  init_bounded_number lalpha(-10.,15.,1)        //log alpha parameter for dirichlet
  init_bounded_number lalpha_FIM(-10.,15.,1)        //log alpha parameter for dirichlet survey

  init_bounded_vector log_fint(fyear,lyear,-20.,0.,1)   //Fishing intensity for each year

 //Derived variables
  vector q(fyear,lyear)                          //catchability
...
///Sigmas
  ...
  number alpha                       //exponentiated log alpha 
  number alpha_FIM                   //exponentiated log alpha for survey

  number alpha_y                       //alpha that changes by year
  number alpha_FIM_y                   //alpha for FIM that changes by year
...
//Dirichlet
  ...
  L2.initialize();
  for(i=fyear;i<=lyear;i++){
//  L2+= nllDirichlet(obs_fishery_comp(i),alpha*pred_fishery_comp(i));
  //Dirichlet by hand
  alpha_y = mfexp(lalpha)*(SS_fishery(i)/mean(SS_fishery));
  if(SS_fishery(i)>0){
  L2 += -gammln(alpha_y)+sum(gammln(alpha_y*pred_fishery_comp(i))-elem_prod((alpha_y*pred_fishery_comp(i)-1),log(obs_fishery_comp(i))));
  }}
...
//Fishery CPUE
  ...
  alpha_FIM=mfexp(lalpha_FIM);
  L4.initialize();
  L5.initialize();
  for(i=lyear*2/5+fyear;i<=lyear;i++){
  alpha_FIM_y=mfexp(lalpha_FIM)*(SS_FIM(i)/mean(SS_FIM(41,100)));
  if(SS_FIM(i)>0){
  L4 += -gammln(alpha_FIM_y)+sum(gammln(alpha_FIM_y*pred_FIM_comp(i))-elem_prod((alpha_FIM_y*pred_FIM_comp(i)-1),log(obs_FIM_comp(i))));       //Likelihood for age composition of FIM survey, will be different from stock diric below because of added constant to suppress zeroes in function below
//  L4+= nllDirichlet(obs_FIM_comp(i),alpha_FIM*pred_FIM_comp(i)); //Dirichlet function
  }
  ...
  open_r_file("SimSCAA.rdat");
   ...
   close_r_vector();
    ...
    open_r_vector("params");
      ...
      wrt_r_item("lalpha",lalpha);              //log alpha parameter for dirichlet
      wrt_r_item("lalpha_FIM",lalpha_FIM);          //log alpha parameter for dirichlet survey
     close_r_vector();
    close_r_file();
