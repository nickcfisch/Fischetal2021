...
PARAMETER_SECTION
  ...
  init_bounded_vector log_fint(fyear,lyear,-10.,0.,1)   //Fishing intensity for each year
...
///Sigmas
  ...
  number cv_FIM_CPUE                  //cv for survey CPUE
...
//  !!cout << fishery_sellogis_midpt << endl;
//  !!exit(88);
...
//   pred_fishery_comp(i)=pred_caa(i)/(pred_totcatch_fishery(i));  //calculating predicted catch age composition
...
//  pred_FIM_comp(i)=pred_FIM_caa(i)/(pred_totcatch_FIM(i));  //calculating predicted survey catch age composition
  }
...
//  L1 += log(cv_fishery*pred_harv(i))+0.5*square((obs_harv(i)-pred_harv(i))/(cv_fishery*pred_harv(i)));   //Normal likelihood using CV
  } 
...
//Multinomial
  ...
  for(i=fyear;i<=lyear;i++){
  if(SS_fishery(i)>0){
  L2 += -1*sum(SS_fishery(i)*elem_prod(obs_fishery_comp(i),log(.000001+pred_fishery_comp(i))));     //Likelihood for age composition of fishery catch
//  L2 += -1*sum(SS_fishery(i)*elem_prod(obs_fishery_comp(i),log(pred_fishery_comp(i))));     //Likelihood for age composition of fishery catch
  }}
...
//  L3 += log(cv_fishery_CPUE*pred_fishery_cpue(i))+0.5*square((obs_fishery_cpue(i)-pred_fishery_cpue(i))/(cv_fishery_CPUE*pred_fishery_cpue(i))); //Likelihood for Fishery CPUE
  }
...
//Fisheries-independent likelihoods
  ...
  if(SS_FIM(i)>0){
  L4 += -1*sum(SS_FIM(i)*elem_prod(obs_FIM_comp(i),log(.000001+pred_FIM_comp(i))));             //Likelihood for age composition of FIM survey
//  L4 += -1*sum(SS_FIM(i)*elem_prod(obs_FIM_comp(i),log(pred_FIM_comp(i))));             //Likelihood for age composition of FIM survey
  }
...
//  L5 += log(cv_FIM_CPUE*pred_totcatch_FIM(i))+0.5*square((obs_FIM_CPUE(i)-pred_totcatch_FIM(i))/(cv_FIM_CPUE*pred_totcatch_FIM(i))); //Likelihood for FIM CPUE
  }
