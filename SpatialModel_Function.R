...
#    Effort<-c(rep(0,nyear_init),Effort_logis_mean[1:50],eff_scalar+(eff_scalar/2)-seq(eff_scalar/2+2,eff_scalar+1,length.out=50))
    Effort<-c(rep(0,nyear_init),Effort_logis_mean[1:50],rep(eff_scalar*0.75,50))
...
#Status Quo Model
#for (i in 1:1){
#Spatial_Model(save_wd=paste0("/blue/edvcamp/nfisch/Spatial_Model/OM_Runs/SQ_NPE_GE_",i),
...
for (i in 1:1){
Spatial_Model(save_wd=paste0("/blue/edvcamp/nfisch/Spatial_Model/OM_Runs/RF_NPE_GE_1010_",i), 
              seed=i,PE=FALSE,nyear=150,nyear_init=50,num_ages=21,lam_data=TRUE,lam_moveCost=0.02,DD_rate=0.5,DD_Thresh_quant=0.75,sig_logR=0.3,
