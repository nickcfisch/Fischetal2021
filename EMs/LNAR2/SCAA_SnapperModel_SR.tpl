...
//Logistic-Normal AR2 Parameterization
...
PARAMETER_SECTION
  ...
  init_bounded_number phi(-2.,2,1)               //Logistic-normal rho parameter for fishery comp
  init_bounded_number phipsi2_logit(-10.,10.,1)               //Logistic-normal rho2 parameter for fishery comp
  init_bounded_number lsig_LN(-5.,5.,1)            //Logistic-normal sigma parameter for fishery comp

  init_bounded_number phi_surv(-2.,2,1)        //Logistic-normal rho parameter for survey comp
  init_bounded_number phipsi2_surv_logit(-10.,10.,1)        //Logistic-normal rho2 parameter for survey comp
...
FUNCTION Logistic_Normal_calcs
  ...
  phipsi2 = mfexp(phipsi2_logit)/(1+mfexp(phipsi2_logit));
  phi2 = -1.+(2-fabs(phi))*phipsi2 ;  //rho 2 bounded calculation
  for(i=fyear;i<=lyear;i++){
  if(SS_fishery(i)>0){
  ...
  rhovec(0) = 1.;
  rhovec(1) = phi/(1-phi2);   //Francis (2014) description of lag 1 is wrong wrt AR2
  for(i=2;i<=lage;i++){
   rhovec(i) = phi*rhovec(i-1)+phi2*rhovec(i-2);
  }
//  cout<<rhovec<<endl;
//  cout<<wts<<endl;
//  exit(1);
...
//////////////////////////////////////////////////////
   phipsi2_surv = mfexp(phipsi2_surv_logit)/(1+mfexp(phipsi2_surv_logit));
   phi2_surv = -1.+(2-fabs(phi_surv))*phipsi2_surv ;  //rho 2 bounded calculation
   ...
   rhovec_surv(0) = 1.;
   rhovec_surv(1) = phi_surv/(1-phi2_surv);
   for(i=2;i<=lage;i++){
    rhovec_surv(i) = phi_surv*rhovec_surv(i-1)+phi2_surv*rhovec_surv(i-2);
   }
  ...
  open_r_file("SimSCAA.rdat");
   ...
   close_r_vector();
    ...
    open_r_vector("params");
      ...
      wrt_r_item("phi2",phi2);               //Logistic-normal phi2 parameter for fishery comp
      ...
      wrt_r_item("phi2_surv",phi2_surv);       //Logistic-normal phi parameter for survey comp
