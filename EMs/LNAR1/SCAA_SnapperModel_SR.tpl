...
//Logistic-Normal AR1 Parameterization
...
PARAMETER_SECTION
  ...
  init_bounded_number phi(-1.,1,1)               //Logistic-normal phi parameter for fishery comp
  init_bounded_number lsig_LN(-5.,5.,1)            //Logistic-normal sigma parameter for fishery comp

  init_bounded_number phi_surv(-1.,1,1)        //Logistic-normal phi parameter for survey comp
  init_bounded_number lsig_LN_surv(-7.,5.,1)      //Logistic-normal sigma parameter for survey comp
...
//Code to generate Covariance matrix each year for multivariate normal variate x
   for(i=fage;i<=lage;i++){
    for(j=fage;j<=lage;j++){
     ...
     covmat_rho(i,j) =  pow(phi,indicator_mat(i,j));       //covariance matrix of only rho
    }
   }
...
//////////////////////////////////////////////////////
   ...
   for(i=fage;i<=lage;i++){
    for(j=fage;j<=lage;j++){
     covmat_surv_rho(i,j) =  pow(phi_surv,indicator_mat(i,j));       //covariance matrix of only rho
    }
   }
  ...
  open_r_file("SimSCAA.rdat");
   ...
   close_r_vector();
    ...
    open_r_vector("params");
      ...
      wrt_r_item("phi",phi);               //Logistic-normal rho parameter for fishery comp
      wrt_r_item("lsig_LN",lsig_LN);          //Logistic-normal sigma parameter for fishery comp
      wrt_r_item("phi_surv",phi_surv);       //Logistic-normal rho parameter for survey comp
